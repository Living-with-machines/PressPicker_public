    ...
    function draw(container) {
       ...
       [earliest_date, latest_date] = d3.extent(Object.keys(files[1][Object.keys(files[1])[0]])
            ...
            newspaper_titles_data.forEach(function (d, i) {
                ...
                // so that the line graph always draws from 0 and ends at 0, add explicit 0 count data for 1 year before and after data
            ...
            data.forEach(function (d, i) {
                if ('connected_titles' in d) {
                    ...
                    d['connected_titles'].forEach(function (nested_d, nested_i) {
                        ...
                        title_wrapper.append('a').attr('xlink:href', nested_d["Link"]).attr('target', '_blank').append('text').attr('class', 'title_id').text(nested_d["Title.ID"]).attr('transform', 'translate(-75,45)');
                    ...
                    title_wrapper.append('a').attr('xlink:href', d["Link"]).attr('target', '_blank').append('text').attr('class', 'title_id').text(d["Title.ID"]).attr('transform', 'translate(-75,45)');
